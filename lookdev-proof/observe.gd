...
const PAYLOAD := "res://generated/building_material_payload.json"
const RECEIPT := "res://material-lookdev-runtime-receipt.json"

var payload: Dictionary = {}
var receipt := {
    "schema": "axm.building-material-lookdev-runtime/v0.1",
    "promotion_effect": "NONE",
    "renderer_boundary": "Godot 4.7.2 GL Compatibility procedural proof representation rebuilt from exact Building component boxes and exact panel receiver frames. This is a controlled material A/B host, not Map integration, architectural validation or runtime/gameplay acceptance."
}
...
func make_component(component: Dictionary, variant: String) -> MeshInstance3D:
    ...
    if String(component["kind"]) != "box":
        push_error("unsupported component kind")
        return null
...
func _initialize() -> void:
    ...
    if payload.get("schema") != "axm.building-material-lookdev-payload/v0.1":
        fail("missing or invalid Building material lookdev payload")
        return
    var rows := {}
    ...
    receipt["state"] = "PASS_TARGET_HOST_BUILDING_SURFACE_AB_READY"
    receipt["contexts"] = rows
    receipt["godot_version"] = Engine.get_version_info()
    receipt["truth_boundary"] = payload["truth_boundary"]
    write_receipt()
    print("AXM BUILDING MATERIAL LOOKDEV ", JSON.stringify(receipt))
    quit(0)
